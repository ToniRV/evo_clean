...
def csv_read_matrix(file_path, delim=',', comment_str="#"):
    ...
    """
    # Handle multiple character delimiter by replacing them for '|'
    # csv.reader only allows for single character delimiters.
    if hasattr(file_path, 'read'):  # if file handle
        generator = (
            line.replace(delim, '|') for line in file_path if not line.startswith(comment_str))
        reader = csv.reader(generator, delimiter='|')
        ...
        with open(file_path) as f:
            generator = (
                line.replace(delim, '|') for line in f if not line.startswith(comment_str))
            reader = csv.reader(generator, delimiter='|')
            mat = [row for row in reader]
    return mat
