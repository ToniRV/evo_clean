...
class PE(Metric):
    ...
    def get_all_statistics(self):
        ...
        """
        statistics = {}
        q1, q2, q3 = np.percentile(self.error, [25, 50, 75])
        for s in StatisticsType:
            if s == StatisticsType.median:
                statistics[s.value] = q2
...
class RPE(PE):
    ...
    def process_data(self, data, id_pairs=None):
        ...
        if self.pose_relation == PoseRelation.translation_part:
            ...
            self.error = np.array([abs(lie.so3_log(lie.so3_from_se3(E_i))) for E_i in self.E])
        elif self.pose_relation == PoseRelation.rotation_angle_deg:
            self.error = np.array([abs(lie.so3_log(lie.so3_from_se3(E_i))) * 180 / np.pi for E_i in self.E])
        else:
